...
## Need to do some data processing to generate the following columns:
##  - StressedOnset
##  - StressedVowel
##  - Coda
##  - Stressed
...
## 1) let's syllabify the words
##    - very simple heuristic: V.V, V.CV, VC.CV, 

hun$syllables <- hun$hun_phonemes
for (i in 1:10) { # this is needed since regexps don't match overlapping sequences
  hun$syllables <- str_replace_all(hun$syllables, "([IiYyEeØøAaOoUu])([IiYyEeØøAaOoUu])", "\\1,\\2")
  hun$syllables <- str_replace_all(hun$syllables, "([IiYyEeØøAaOoUu])([ptckbdDgTJfsShvzZmnHlrj][IiYyEeØøAaOoUu])", "\\1,\\2")
  hun$syllables <- str_replace_all(hun$syllables, "([IiYyEeØøAaOoUu][ptckbdDgTJfsShvzZmnHlrj])([ptckbdDgTJfsShvzZmnHlrj][IiYyEeØøAaOoUu])", "\\1,\\2")
}
hun$syllables <- paste(hun$syllables, ",", sep="")

hun <- mutate(hun,
              StressedOnset = str_match(hun_phonemes, "^(.*?)[IiYyEeØøAaOoUu]")[,2],
              StressedVowel = str_match(hun_phonemes, "^.*?([IiYyEeØøAaOoUu])")[,2],
              Coda = unlist(lapply(strsplit(syllables, ","), function (x) {paste(str_match(x, "^.*?[IiYyEeØøAaOoUu](.*?)$")[,2], collapse="")})),
              Stressed = str_match(syllables, "^(.*?),")[,2],
              Consonantal = gsub("[IiYyEeØøAaOoUu]","",hun_phonemes))

##--------------------------------------------------------------------------------
## Descriptive stats before anything else (because the Mantel tests *don't* work)
##--------------------------------------------------------------------------------

hun$r <- get_consonants(hun$hun_phonemes)[,"r"] > 0
hun$l <- get_consonants(hun$hun_phonemes)[,"l"] > 0

# note that this is different from the definition in the random forests script,
# where the cutoff for roughness is 1, not 0
hun$RoughCat = ifelse(hun$hun_roughness > 0, 'rough', 'smooth') 

prop.table(table(hun$RoughCat, hun$r), 1)
prop.table(table(hun$RoughCat, hun$l), 1)

# ... and in non-IE:

hun.non.ie <- hun[!(hun$etymology_source %in% c("slavic","germanic","greek","romance")),]

prop.table(table(hun.non.ie$RoughCat, hun.non.ie$r), 1)
prop.table(table(hun.non.ie$RoughCat, hun.non.ie$l), 1)

# rudimentary boxplot

hun.non.ie$is.r <- ifelse(hun.non.ie$r, "R", "no R")
boxplot(hun_roughness ~ is.r, data=hun.non.ie, ylab="roughness rating")
...
source(file.path(mainPath, 'phoneme_extraction_hun.R'))
...
onsetCons <- get_consonants(hun$StressedOnset)
onsetVowel <- get_vowels(hun$StressedVowel)
onsetWithVowel <- cbind(onsetCons, onsetVowel)
codaCons <- get_consonants(hun$Coda)
fullCons <- get_consonants(hun$hun_phonemes)
fullStress <- get_consonants(hun$Stressed)
fullSyll <- cbind(fullStress, get_vowels(hun$Stressed))
...
rough.dist <- dist(hun$hun_roughness,
	method = 'euclidian')

## Compute Mantel test correlations:

mantel.randtest(rough.dist, onsetCons.dist)	# r = -0.02 
mantel.randtest(rough.dist, onsetVowel.dist)	# r = 0.06 *
mantel.randtest(rough.dist, onsetWithVowel.dist)	# r = 0.03
mantel.randtest(rough.dist, codaCons.dist)	# r = -0.05
mantel.randtest(rough.dist, fullStress.dist)	# r = -0.04
mantel.randtest(rough.dist, fullSyll.dist)	# r = - 0.0
mantel.randtest(rough.dist, fullCons.dist) # r = - 0.025

## Do the same thing with Levenshtein edit distances:

onsetCons.lev <- as.dist(adist(hun$StressedOnset))
onsetVowel.lev <- as.dist(adist(hun$StressedVowel))
onsetWithVowel.lev <- as.dist(adist(paste0(hun$Coda, hun$StressedVowel)))
codaCons.lev <- as.dist(adist(hun$Coda))
fullCons.lev <- as.dist(adist(hun$Consonantal))
fullStress.lev <- as.dist(adist(hun$Stressed))
fullSyll.lev <- as.dist(adist(paste0(hun$Stressed, hun$StressedVowel)))

#adist(paste0(hun$StressedOnset, hun$StressedVowel))

## Perform correlations with Levenshtein edit distances:

mantel.randtest(rough.dist, onsetCons.lev)	# r = 0.04
mantel.randtest(rough.dist, onsetVowel.lev)	# r = 0.06 *
mantel.randtest(rough.dist, onsetWithVowel.lev)	# r = 0.01
mantel.randtest(rough.dist, codaCons.lev)	# r = -0.01
mantel.randtest(rough.dist, fullCons.lev)	# r = 0.01
mantel.randtest(rough.dist, fullStress.lev)	# r = 0.01
mantel.randtest(rough.dist, fullSyll.lev)	# r = 0.01




###
### NOTE FOR BODO: I've only edited the script up to this point - the rest is all your stuff
###
